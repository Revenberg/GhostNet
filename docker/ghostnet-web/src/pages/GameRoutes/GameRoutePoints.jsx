...
export default function GameRoutePoints() {
    ...
    useEffect(() => {
        ...
        async function fetchPoints() {
            const backendHost = process.env.REACT_APP_BACKEND_URL || "http://localhost:4000";
            try {
                const res = await fetch(`${backendHost}/api/game_route/points/by-game/${selectedGameId}`);
                const data = await res.json();
    ...
    const handleDelete = async id => {
        ...
        const backendHost = process.env.REACT_APP_BACKEND_URL || "http://localhost:4000";
    const res = await fetch(`${backendHost}/api/game_route/points/${id}`, { method: "DELETE" });
    ...
    const url = editingId ? `${backendHost}/api/game_route/points/${editingId}` : `${backendHost}/api/game_route/points`;
